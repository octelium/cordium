...
func TestCancelBuildTemplate(t *testing.T) {
	...
	t.Run("a Template with no BuildInfo is a no-op", func(t *testing.T) {
		tmpl.Status.BuildInfo = nil
		tmpl, err = srv.octeliumC.CordiumC().UpdateTemplate(context.Background(), tmpl)
		assert.Nil(t, err, "%+v", err)
		assert.Nil(t, tmpl.Status.BuildInfo)

		ret, err := srv.CancelBuildTemplate(usr.Ctx(), &cordiumv1.CancelBuildTemplateRequest{
			TemplateRef: umetav1.GetObjectReference(tmpl),
		})
		assert.Nil(t, err, "%+v", err)
		assert.Equal(t, tmpl.Metadata.Uid, ret.Metadata.Uid)
	})

	t.Run("a non-admin Member cannot cancel a build", func(t *testing.T) {
		...
		_, err = srv.CreateMembership(usr.Ctx(), &cordiumv1.CreateMembershipRequest{
		...
		_, err = srv.CancelBuildTemplate(usr2.Ctx(), &cordiumv1.CancelBuildTemplateRequest{
			TemplateRef: umetav1.GetObjectReference(tmpl),
		})
		assert.NotNil(t, err)
		assert.True(t, grpcerr.IsUnauthorized(err), "%+v", err)

		_, err = srv.BuildTemplate(usr2.Ctx(), &cordiumv1.BuildTemplateRequest{
			TemplateRef: umetav1.GetObjectReference(tmpl),
		})
		assert.NotNil(t, err)
		assert.True(t, grpcerr.IsUnauthorized(err), "%+v", err)
