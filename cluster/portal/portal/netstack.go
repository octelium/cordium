...
func createNetstackTUN(v4addr *net.IPNet) (*netTun, error) {
...

	{
		...
		if err := dev.stack.AddProtocolAddress(1, tcpip.ProtocolAddress{
			...
			AddressWithPrefix: tcpip.AddrFromSlice(v4addr.IP.To4()).WithPrefix(),
		}, stack.AddressProperties{}); err != nil {
			return nil, errors.Errorf("Could not add addr v4: %+v", err)
		}
	}
...
func (tnet *Net) tryOneName(ctx context.Context, name string, qtype dnsmessage.Type) (dnsmessage.Parser, string, error) {
	...
	dnsServers = append(dnsServers, net.ParseIP("8.8.8.8"))
